...
export default function Sidebar({ currentPage, onNavigate, onLogout, isMobile = false, onCloseMobile }: SidebarProps) {
  ...
  return (
    <div
      className={`${
        isMobile ? "w-72" : sidebarOpen ? "w-56" : "w-16"
    ...
    >
      ...
      <div className="p-4 mb-4">
        <div className={`flex items-center gap-3 transition-all duration-300 ${!isExpanded && "justify-center"}`}>
          ...
          <div className={`flex flex-col transition-all duration-300 ${!isExpanded ? "opacity-0 invisible w-0" : "opacity-100"}`}>
      ...
      <nav className="flex-1 px-3 space-y-1">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`flex items-center ${!isExpanded && "mx-auto"}`}>
                {Icon && <Icon className={`w-4 h-4 transition-transform duration-200 ${isExpanded && "mr-3"} ${isActive ? "scale-100" : "group-hover:scale-110"}`} />}
                <span className={`text-xs font-semibold transition-all duration-200 whitespace-nowrap ${!isExpanded ? "opacity-0 w-0" : "opacity-100"}`}>
                  {item.label}
                </span>
              </div>
            </button>
      ...
      <div className="p-4 border-t border-slate-800/50">
        <div className="flex flex-col gap-2">
          <button
            ...
            className={`flex items-center justify-center p-2 hover:bg-slate-800 rounded-lg transition-all text-slate-500 hover:text-slate-300 ${isMobile ? "hidden" : ""}`}
          >
          ...
          >
            <LogOut className={`w-4 h-4 transition-all duration-200 ${isExpanded && "mr-3"}`} />
            <span className={`font-bold text-[10px] tracking-wider uppercase ${!isExpanded && "hidden"}`}>
              Log Out
            </span>
          </button>
        </div>
